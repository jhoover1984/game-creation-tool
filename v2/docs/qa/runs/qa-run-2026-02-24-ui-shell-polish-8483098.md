...
- 1366x768: Pass
- 1920x1080: Pass 
- 2560x1440: Env limitation - DevTools emulation caused page scroll/overflow artifact; requires real 2560x1440 window/monitor verification
...
**Result:** Fail - tabs are small and truncated see 'qa-shell-1920x1080.png'
...
**Result:**
- 1366x768: Fail - the inspector doesn't look like it overlaps but right side of canvas incomplete
- 1920x1080: Fail - the inspector doesn't look like it overlaps but right side of canvas incomplete
- 2560x1440: Pass - can see fully the right side of the canvas
...
**Result:** Pass (when clicking save it doesn't have any feedback or pop-ups showing it saved, clicking load just loads with no idiciation of what you loaded. I have to use the mouse wheel to zoom the map out then if i click fit it goes back to default size)
...
**Result:** Fail - depending on window size the right side of the canvas cannot be complete so it may show that last column of squares being cut off or showing the the actual border (qa-900px-window.png)
...
**Result:** Pass ( there is no tags field, you have to open up the meta data to see the name field that says player)
...
**Result:** Pass (Metadata is the only one that shows up as being closed automatically so you need to open it to see the options)
...
**Result:** Pass (player added spawns it in the center of the map inbetween squares)
...
**Result:** Pass - solid is centered in the panel for some reason and changing coordinates manually does nothing, you can select or drag the entity around
...
**Result:** Fail - can see playtest: running in the upper right and lower left of hud but nothing else and the play/pause states
...
**Result:** Fail - console shows when playtest starts and stops but there is not tick mark or timer that shows
...
**Result:** Fail - no tick numbers show up anywhere on hud or console, player not able to move even with non-zero speed
...
**Evidence:** Reused `qa-shell-1920x1080.png` (HUD visible in header).
...
**Result:** Pass (just keeps putting 'queued interacted input for next step')
...
**Result:** Pass - this looks bad and too basic and you cannot move the player during playtesting or anything.
...
4. Reopen `http://localhost:4173/ui-editor/src/app.html` in a new tab.
...
**Result:** Pass (when selecting the option to always dismiss pop-up it shouldn't just disappear, should tick it and have to submit that or save that option. Also, this modal might change later since we will have a dashboard which would than have maybe a pop-up or something like other tools or engines have with some hints or things about the editor)
...
**Result:** Pass (tabs on left panel are very small as they are all at the top horizontally and you only see the first lettter of each tab)
...
**Setup:** 1920x1080. A diagnostic must be triggered. Use one of these methods:
  - Option A: After SF-01, move the player entity to a canvas edge position so it is out of bounds.
  - Option B: In the toolbar, enter an invalid map size (e.g. W=0) and click Apply Map.
  - Option C: Paint a tile on a position outside the map bounds if exposed via rule paint.
...
**Result:** Fail - Option A: I don't understand allowing manual map sizes like we do since if you change tile sizes it sort of breaks the boundaries where whole squares on the grid are cut in half and you can still place an entitiy in those squares where you only see half of it.   Option B: Input w=0 and hitting apply map doesn't do anything not issues or diagonostics pop up and the map doesn't change. Option C: I can't use the rule paint to paint outside the bounds.
...
| `qa-shell-1920x1080.png` (reused) | PT-04 | Playtest HUD visible in header area |
...
|----|----------------------|-----------|-------------|--------------------------|--------|
| D-001 | high | SH-02 | Open app at 1920x1080, inspect left rail tabs; labels appear truncated and overly small | `packages/ui-editor/src/styles/layout.css`, tab-bar styles | TBD |
| D-002 | high | CV-01/CV-02/CV-05 | At 1366x768 and 1920x1080, right canvas edge appears clipped/incomplete on startup and during resize; 2560x1440 passes | viewport fit math + canvas-stage constraints (`EditorShellController` / `ViewportController`) | TBD |
| D-005a | high | PT-01/PT-02/PT-03 | Playtest HUD shows running state but expected tick/position details are not visible; step/tick verification not possible | playtest HUD data rendering/binding in shell | TBD |
| D-005b | medium | PT-03 | Movement expectation unclear: player movement requires keyboard input during playtest but UI provides no clear affordance | playtest UX copy/hints | TBD |
| D-006 | low | TC-03 (runbook maintenance) | TC-03 Option B (`Map W=0`) did not produce reliable diagnostic; checklist trigger path needs update to a reliable repro | `docs/runbooks/Manual QA Checklist.md` TC-03 setup | TBD |
| D-007 | medium | IN-04 | Editing Transform coordinates manually did not move entity position in canvas | inspector apply path / entity move command wiring | TBD |
| D-008 | low | IN-03 | Player spawn appears on gridline between tile squares at center spawn | Add Player / Add Starter Scene spawn position logic | TBD |
| D-009 | low | SF-03 | Tags discoverability is weak because Metadata is collapsed by default; player tag confirmation is not above-the-fold | inspector default foldout behavior/copy | TBD |
...
| Date   | 2/25/26                            |
| Result | Fail                               |
| Notes  | SHA under test: 8483098. Main blockers: SH-02, D-002 (CV-01/CV-02/CV-05), and D-005a (PT-01/PT-02/PT-03). 2560x1440 SH-01 issue treated as DevTools emulation limitation, not product failure. |
