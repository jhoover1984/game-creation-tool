...
function renderTargetChip(target) {
    if (target.type === 'this')
        return '<span class="behavior-chip">This entity</span>';
    if (target.type === 'tag')
        return `<span class="behavior-chip">tag: ${escapeHtml(target.value)}</span>`;
...
function renderEditMode(entityId, rows) {
    if (rows.length === 0) {
...
function renderTraceEntry(entry) {
    const condHtml = entry.conditionResults.map((c) => `<span class="${c.passed ? 'trace-pass' : 'trace-fail'}" title="${escapeHtml(c.reason)}">${escapeHtml(c.type)}</span>`).join(' ');
    const actHtml = entry.actionResults.map((a) => `<span class="${a.dispatched ? 'trace-pass' : 'trace-fail'}" title="${escapeHtml(a.reason)}">${escapeHtml(a.type)}</span>`).join(' ');
...
function renderPlaytestMode(trace) {
    if (trace.length === 0) {
 ...
 */
export function renderBehaviorPanel(entityId, rows, mode, trace) {
    if (!entityId) {
...
//# sourceMappingURL=behavior-panel.js.map