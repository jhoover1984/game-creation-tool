...
function storyProjectJson(nodes, edges) {
...
test('UI-TASKS-002: missing start node generates task with fix action', () => {
    ...
    app.diagnosticStore.ingestSemanticDiagnostics([{ code: 'QUEST_START_NODE_MISSING', severity: 'error', path: '/nodes', message: 'No start node found.' }], 'project-load');
    const tasks = app.getTasks();
...
test('UI-TASKS-002: each task references a valid diagnostic', () => {
    ...
    for (const task of tasks) {
        assert.ok(diags.some((d) => d.id === task.diagnosticId), `task ${task.id} must reference a valid diagnostic`);
    }
});
...
test('EDIT_DUPLICATE_ENTITY_NAME clears after renaming duplicate to unique name', () => {
    ...
    assert.equal(app.getDiagnostics().filter((d) => d.code === 'EDIT_DUPLICATE_ENTITY_NAME').length, 0, 'duplicate diagnostic should clear after rename');
});
test('deterministic duplicate-name fix renames and clears diagnostics', () => {
    ...
    assert.equal(app.getDiagnostics().filter((d) => d.code === 'EDIT_DUPLICATE_ENTITY_NAME').length, 0, 'duplicate diagnostics should be resolved by deterministic fixes');
});
...
test('EDIT_DUPLICATE_ENTITY_NAME clears after deleting one of two same-named entities', () => {
    ...
    app.createEntity('Clone', 8, 8);
    app.createEntity('Clone', 16, 16);
    assert.ok(app.getDiagnostics().some((d) => d.code === 'EDIT_DUPLICATE_ENTITY_NAME'), 'should have duplicate diagnostic');
    const secondId = app.store.entities[1].id;
    app.deleteEntity(secondId);
    assert.equal(app.getDiagnostics().filter((d) => d.code === 'EDIT_DUPLICATE_ENTITY_NAME').length, 0, 'duplicate diagnostic should clear after deleting one duplicate');
});
...
//# sourceMappingURL=editor-app.test.js.map