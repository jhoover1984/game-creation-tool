...
function makeEntity(id, tags = []) {
    return { id, name: id, position: { x: 0, y: 0 }, size: { w: 16, h: 16 }, solid: false, tags };
}
function makeOnTickRow(id, conditionType, tagValue = '') {
    return {
        id,
        label: 'Row',
        enabled: true,
...
test('BEHAV-DEBUG-001: disabled rows are skipped by evaluator', () => {
    const evaluator = new BehaviorEvaluator();
    const entity = makeEntity('e1');
    const row = { ...makeOnTickRow('r1', 'always'), enabled: false };
    const entries = evaluator.evaluate({ e1: [row] }, [entity], 'on:tick');
...
test('BEHAV-DEBUG-001: triggerEntityIds restricts evaluation for contextual triggers', () => {
    ...
    const row1 = { ...makeOnTickRow('r1', 'always'), trigger: { type: 'on:interact' } };
    const row2 = { ...makeOnTickRow('r2', 'always'), trigger: { type: 'on:interact' } };
    const entries = evaluator.evaluate({ e1: [row1], e2: [row2] }, [e1, e2], 'on:interact', new Set(['e2']));
    assert.equal(entries.length, 1);
...
test('BEHAV-DEBUG-001: entity_in_radius condition passes when another entity is nearby', () => {
    ...
    const entries = evaluator.evaluate({ e1: [row] }, [
...
//# sourceMappingURL=behavior-debug-trace.test.js.map