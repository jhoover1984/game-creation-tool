...
describe('renderEntityInspector -- field rendering', () => {
  ...
  test('renders id field as readonly', () => {
    ...
    assert.match(
      html,
      /data-path="id"[^>]*readonly/,
      'id field must have readonly attribute',
    );
  });
...
describe('renderEntityInspector -- section foldouts', () => {
  ...
  test('renders Metadata section without open attribute (collapsed by default)', () => {
    ...
    assert.doesNotMatch(
      html,
      /<details class="inspector-section" open>[^<]*<summary[^>]*>Metadata/,
      'Metadata section must not be open by default',
    );
  });
  ...
  test('position.x is inside Transform section', () => {
    ...
    assert.ok(
      transformIdx < posXIdx,
      'position.x field must appear after Transform section header',
    );
  });
  ...
  test('id field is inside Metadata section', () => {
    ...
    assert.ok(
      metadataIdx < idIdx,
      'id field must appear after Metadata section header',
    );
  });
