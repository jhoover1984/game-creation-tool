...
describe('UI-VISUAL-003: token compliance gate', () => {
  it('no --prim-* references outside tokens.css', () => {
    ...
    assert.ok(
      match === null,
      `--prim-* reference found outside tokens.css: "${match?.[0]}"`,
    );
  });
  ...
  it('no raw hex values outside tokens.css', () => {
    ...
    assert.ok(
      match === null,
      `Raw hex found outside tokens.css: "${match?.[0]}"`,
    );
  });
  ...
  it('canvas uses inset box-shadow for framing, not layout border -- UI-SHELL-POLISH-001 A5', () => {
    ...
    assert.ok(
      !canvasBlock.includes('border:'),
      'canvas must not use border: (adds layout width and causes right-edge clip)',
    );
...
describe('UI-SHELL-POLISH-001: shell structure compliance', () => {
  it('btn-add-starter has btn--primary class (A1 primary action)', () => {
    ...
    assert.ok(
      starterMatch[0].includes('btn--primary'),
      'btn-add-starter must have btn--primary class for visual hierarchy',
    );
  });

  it('btn-add-starter text is "Add Starter Scene" (A1 copy alignment)', () => {
    assert.ok(
      appHtml.includes('Add Starter Scene'),
      '"Add Starter Scene" copy must be present in app.html',
    );
  });
  ...
  it('btn-add-player has btn--secondary class (A1 secondary action)', () => {
    ...
    assert.ok(
      playerMatch[0].includes('btn--secondary'),
      'btn-add-player must have btn--secondary class for visual hierarchy',
    );
  });
  ...
  it('toolbar has Apply Map button with btn--secondary class (A6)', () => {
    ...
    assert.ok(
      applyMatch[0].includes('btn--secondary'),
      'btn-apply-map must have btn--secondary class',
    );
  });
});
