...
import type { StoryPanelAppAdapter } from './story-panel-controller.js';
import type { QuestGraphNode } from '@gcs/contracts';

function makeNodes(): QuestGraphNode[] {
  return [
...
function makeContainer(fields: Record<string, { value?: string }>) {
  ...
  return container as unknown as HTMLElement & {
    fireApply(): void;
    fireNodeChange(value: string): void;
  };
}

function makeApp(nodes = makeNodes()): StoryPanelAppAdapter & {
  selectedId: string | null;
  updates: Array<{ nodeId: string; name: string; kind: QuestGraphNode['kind'] }>;
} {
  const updates: Array<{ nodeId: string; name: string; kind: QuestGraphNode['kind'] }> = [];
  let selectedId: string | null = nodes[0]?.nodeId ?? null;
  return {
    updates,
    get selectedId() {
      return selectedId;
    ...
    selectQuestNode(nodeId: string | null) {
      selectedId = nodeId;
