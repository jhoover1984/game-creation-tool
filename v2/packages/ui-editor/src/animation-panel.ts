import type { AnimationClipDef, EntityDef, OcclusionHint, SlotAttachment } from '@gcs/contracts';
...
function renderAnchorRows(clip: AnimationClipDef): string {
...
function renderSlotRows(entity: EntityDef): string {
  ...
  const occlusion = (slot: SlotAttachment, value: OcclusionHint): string =>
    slot.occlusionHint === value ? ' selected' : '';
  return `<ul class="anim-slot-list">` +
    slots.map((slot) =>
      `<li class="anim-slot-row">` +
...
export function renderAnimationPanel(
  selectedEntity: EntityDef | null,
  clip: AnimationClipDef | null,
