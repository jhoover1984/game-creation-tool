...
function parseTabLabels(html: string): Array<{ dataTab: string; label: string }> {
  const results: Array<{ dataTab: string; label: string }> = [];
...
test('D-001: all 9 tab buttons are present in app.html', () => {
  ...
  assert.equal(
    labels.length,
    TAB_NAMES.length,
    `expected ${TAB_NAMES.length} tab buttons but found ${labels.length}`,
  );
  for (const name of TAB_NAMES) {
    assert.ok(
      labels.some((l) => l.dataTab === name),
      `tab with data-tab="${name}" not found in app.html`,
    );
  }
});
...
test('D-001: no tab button label is a single character (no single-char truncation regression)', () => {
  ...
  for (const { dataTab, label } of labels) {
    assert.ok(
      label.length > 1,
      `tab data-tab="${dataTab}" has label "${label}" with length ${label.length} -- must be > 1`,
    );
  }
});
...
test('D-001: all 9 tab buttons are independently clickable (each has aria-selected attribute)', () => {
  ...
  for (const name of TAB_NAMES) {
    ...
    assert.equal(
      btn.attrs['aria-selected'],
      'true',
      `clicking tab "${name}" should set aria-selected=true`,
    );
  }
});
