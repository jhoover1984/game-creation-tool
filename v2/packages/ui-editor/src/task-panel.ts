...
function severityLabel(severity: EditorTask['severity']): string {
  switch (severity) {
    case 'fatal':
      return 'Fatal';
    case 'error':
...
function categoryLabel(category: EditorTask['category']): string {
  switch (category) {
    case 'topology':
      return 'Topology';
    case 'reference':
 ...
 */
export function renderTasksPanel(tasks: readonly EditorTask[]): string {
  if (tasks.length === 0) {
 ...
 */
export function applyTaskFixById(
