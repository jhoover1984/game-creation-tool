...
describe('UI-VISUAL-002 Slice B: density decoupling', () => {
  it('[data-density="dense"] block exists in layout.css', () => {
    assert.ok(
      layoutCss.includes('[data-density="dense"]'),
      '[data-density="dense"] selector not found in layout.css',
    );
  });
  ...
  it('[data-mode="pro"] block absent from layout.css (density decoupled)', () => {
    assert.ok(
      !layoutCss.includes('[data-mode="pro"]'),
      '[data-mode="pro"] still present in layout.css -- density vars not yet decoupled',
    );
  });
});
