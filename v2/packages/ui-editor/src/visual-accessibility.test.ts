...
describe('UI-VISUAL-004: accessibility normalization', () => {
  ...
  it('reduced-motion block clamps transition-duration to 0.01ms', () => {
    assert.ok(
      a11yCss.includes('transition-duration: 0.01ms'),
      'reduced-motion transition-duration clamp missing',
    );
  });

  it('reduced-motion block clamps animation-duration to 0.01ms', () => {
    assert.ok(
      a11yCss.includes('animation-duration:  0.01ms'),
      'reduced-motion animation-duration clamp missing',
    );
  });
  ...
  it('centralized :disabled rule uses var(--opacity-disabled) in accessibility.css', () => {
    assert.ok(
      a11yCss.includes('var(--opacity-disabled)'),
      'centralized :disabled opacity rule missing from accessibility.css',
    );
  });
  ...
  it('button:hover uses :not(:disabled) guard in base.css', () => {
    assert.ok(
      baseCss.includes('button:hover:not(:disabled)'),
      'button:hover does not guard against :disabled in base.css',
    );
  });
  ...
  it('.dash-recent-card:hover exists in workspaces.css', () => {
    assert.ok(
      workspacesCss.includes('.dash-recent-card:hover'),
      '.dash-recent-card:hover rule missing from workspaces.css',
    );
  });
});
