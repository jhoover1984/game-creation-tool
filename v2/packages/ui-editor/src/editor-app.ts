...
export class EditorApp {
  ...
  runExportPreflight(): ExportPreflightReport {
    ...
    const report = evaluateExportPreflight(
      this.store.manifest,
      this.store.tileLayers,
      this.store.entities,
    );
   ...
   */
  runExportBuild(seed = 0): ExportBuildReport {
  ...
  paintTile(layerId: string, x: number, y: number, tileId: number): void {
    ...
    if (x < 0 || y < 0 || x >= layer.width || y >= layer.height) {
      ...
      this.addEditorDiagnostic(
        'EDIT_TILE_OUT_OF_BOUNDS',
        `/tileLayers/${layerId}/tiles`,
        'Tile position is outside layer bounds.',
        {
  ...
  applyRulePaint(
    layerId: string,
    userCells: readonly { x: number; y: number }[],
    ruleSet: TileRuleSet,
  ): void {
    if (userCells.length === 0) return;
    const layer = this.store.tileLayers.find((l) => l.id === layerId);
    if (!layer) return;
    ...
    for (const { x, y } of neighborhood) {
      ...
      if (!ruleSet.matchTileIds.includes(tile)) continue;
  ...
  updateEntityVisual(
    ...
    if (!spriteIsValid) {
      this.addEditorDiagnostic(
        'EDIT_ASSET_REF_INVALID',
        `/entities/${entityId}/spriteId`,
        'Invalid sprite asset reference format.',
        {
    ...
    if (!animationIsValid) {
      this.addEditorDiagnostic(
        'EDIT_ASSET_REF_INVALID',
        `/entities/${entityId}/animationClipId`,
        'Invalid animation asset reference format.',
        {
  ...
  getSelectedEntity(): EntityDef | null {
    if (!this.store.selectedEntityId) return null;
   ...
   */
  applyEffectPreset(
    presetId: import('@gcs/contracts').EffectPresetId | null,
    intensity: number,
  ): boolean {
    ...
    if (!event) {
      this.addEditorDiagnostic(
        'EFFECT_PRESET_UNKNOWN',
        '/effects/preset',
        'Unknown effect preset ID.',
        undefined,
        'warning',
      );
      return false;
   ...
   */
  setEffectFieldCoupling(
    fieldId: import('@gcs/contracts').EffectFieldId | null,
    influence: number,
  ): boolean {
    ...
    if (!event) {
      this.addEditorDiagnostic(
        'EFFECT_FIELD_UNKNOWN',
        '/effects/fieldLink',
        'Unknown effect field ID.',
        undefined,
        'warning',
      );
      return false;
  ...
  applyFix(task: EditorTask): boolean {
    if (!task.fixAction) return false;
    if (!task.fixAction.deterministic) return false;
    const diagnostic = this.diagnosticStore.getAll().find((d) => d.id === task.diagnosticId);
    if (!diagnostic) return false;
    if (!task.fixAction.commandType || !task.fixAction.commandPayload) return false;
  ...
  private checkEntityBounds(entityId: string): void {
    const entity = this.store.entities.find((e) => e.id === entityId);
    if (!entity) return;
    ...
    if (
      ...
      this.addEditorDiagnostic(
        'EDIT_ENTITY_OUT_OF_BOUNDS',
        `/entities/${entityId}`,
        'Entity extends beyond canvas bounds.',
        {
  ...
  private refreshDuplicateNameDiagnostics(): void {
  ...
  private suggestUniqueEntityName(entityId: string, baseName: string): string {
    const used = new Set(
      this.store.entities.filter((e) => e.id !== entityId).map((e) => e.name),
    );
  ...
  private isValidAssetRef(assetId: string | undefined): boolean {
    if (!assetId) return true;
    return /^asset[-_][A-Za-z0-9:_-]{1,126}$/.test(assetId);
  }

  private clearExportPreflightDiagnostics(): void {
    const exportCodes = [
