...
function isDirty(shell: EditorShellController): boolean {
  return (shell as unknown as { isDirty: boolean }).isDirty;
}
...
function makeMinimalElements(extraElements: Record<string, unknown> = {}) {
  const canvas = {
    style: { cursor: '' },
...
test('UI-DIRTY-001: fresh project is clean (isDirty === false on init)', () => {
  const shell = new EditorShellController(makeMinimalElements() as never);
...
test('UI-DIRTY-001: undo back to saved baseline returns clean', () => {
  ...
  (shell as unknown as { recomputeDirty(): void }).recomputeDirty();
...
test('UI-DIRTY-001: markClean after save -- project is clean', () => {
  ...
  (shell as unknown as { markClean(): void }).markClean();
...
test('UI-DIRTY-001: undo past a previous save baseline keeps project dirty', () => {
  ...
  (shell as unknown as { markClean(): void }).markClean(); // save here: baseline includes A
...
test('UI-DIRTY-001: StoryPanelController calls onMutate after successful apply', () => {
  ...
  const mockNode = { nodeId: 'n1', name: 'Start', kind: 'start' as const };
  const mockApp = {
    getQuestNodes: () => [mockNode],
    getSelectedQuestNode: () => mockNode,
    selectQuestNode: () => mockNode,
    updateQuestNodeBasics: (_id: string, fields: { name: string }) => {
  ...
  const container = {
    ...
    querySelector: (sel: string) => {
      if (sel === '[data-path="kind"]') return { value: 'start' };
      if (sel === 'input[data-path="name"]') return { value: 'Renamed' };
      return null;
    },
    querySelectorAll: () => [],
  };

  new StoryPanelController(mockApp as never, container as never, () => { mutateCount++; });

  // Simulate clicking the apply button
  const applyTarget = {
    getAttribute: (name: string) => name === 'data-action' ? 'apply-story-inspector' : null,
  };
  clickListeners.forEach((fn) => fn({ target: applyTarget } as unknown as Event));
...
test('UI-DIRTY-001: StoryPanelController does not call onMutate when updateQuestNodeBasics fails', () => {
  ...
  const mockApp = {
    getQuestNodes: () => [{ nodeId: 'n1', name: 'Start', kind: 'start' as const }],
    getSelectedQuestNode: () => ({ nodeId: 'n1', name: 'Start', kind: 'start' as const }),
