...
function makeUndoRedoButtons() {
  const btnUndo = { disabled: true, clicked: false, addEventListener: (_t: string, fn: () => void) => { btnUndo._fn = fn; }, _fn: (() => undefined) as () => void };
  const btnRedo = { disabled: true, clicked: false, addEventListener: (_t: string, fn: () => void) => { btnRedo._fn = fn; }, _fn: (() => undefined) as () => void };
  return { btnUndo, btnRedo };
}

function makeMinimalElements(btnUndo: unknown, btnRedo: unknown) {
  const canvas = {
    style: { cursor: '' },
