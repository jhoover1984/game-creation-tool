...
describe('UI-VIEWPORT-001: ViewportController', () => {
  ...
  describe('fitToMap', () => {
    ...
    it('D-002: margin param -- right edge stays within (containerWidth - margin*2)', () => {
      ...
      assert.ok(
        scaledWidth <= 1420 - margin * 2,
        `scaled map width ${scaledWidth.toFixed(2)} must be <= ${1420 - margin * 2}`,
      );
    });
    ...
    it('D-002: margin param -- bottom edge stays within (containerHeight - margin*2)', () => {
      ...
      assert.ok(
        scaledHeight <= 800 - margin * 2,
        `scaled map height ${scaledHeight.toFixed(2)} must be <= ${800 - margin * 2}`,
      );
    });
    ...
    it('D-002: margin param -- panX >= margin so both sides get clearance', () => {
      ...
      assert.ok(
        vc.panX >= margin,
        `panX ${vc.panX.toFixed(2)} should be >= margin ${margin}`,
      );
    });
