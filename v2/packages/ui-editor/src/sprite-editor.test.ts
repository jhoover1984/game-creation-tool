...
function makeStore(assetId = 'spr-1', w = 4, h = 4): SpriteWorkspaceStore {
  const s = new SpriteWorkspaceStore();
  s.openSprite(assetId, w, h);
  return s;
}

function getPixel(store: SpriteWorkspaceStore, x: number, y: number): [number, number, number, number] {
  const buf = store.getActiveBuffer()!;
  const idx = (y * buf.width + x) * 4;
  return [buf.pixels[idx], buf.pixels[idx + 1], buf.pixels[idx + 2], buf.pixels[idx + 3]] as [number, number, number, number];
}
