...
export class DiagnosticStore {
  ...
  removeByCodeAndPath(code: string, pathPrefix: string): void {
    ...
    this.diagnostics = this.diagnostics.filter(
      (d) => !(d.code === code && d.path.startsWith(pathPrefix)),
    );
    if (this.diagnostics.length !== before) {
      this.notify();
    }
   ...
   */
  ingestSemanticDiagnostics(
    semanticDiags: SemanticDiagnostic[],
...
function resolveDiagnosticCategory(code: string, source: DiagnosticSource): DiagnosticCategory {
  if (source === 'runtime') return 'runtime';
  if (source === 'schema') return 'validation';
  if (source === 'semantic' || source === 'project-load') {
    if (code.includes('UNREACHABLE') || code.includes('START_NODE') || code.includes('EDGE')) return 'topology';
    if (code.includes('DUPLICATE')) return 'reference';
    return 'validation';
  }
  if (source === 'editor') {
    if (code.includes('BOUNDS')) return 'bounds';
    if (code.includes('ASSET_REF')) return 'reference';
    if (code.includes('LAYER') || code.includes('ENTITY') || code.includes('COMMAND')) return 'workflow';
    return 'interaction';
  }
  return 'unknown';
}
 ...
 */
export function resolveFixAction(code: string): FixAction | undefined {
  switch (code) {
 ...
 */
function taskLabelForCode(code: string, fallbackMessage: string): string {
  switch (code) {
    case 'QUEST_DUPLICATE_NODE_ID':
