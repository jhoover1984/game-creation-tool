...
/**
 * Internal undo record. Captures exactly the data needed to reverse a command.
 * Commands (from contracts) describe intent; UndoRecords describe reversal.
 */
type UndoRecord =
  | { type: 'tile:set'; layerId: string; x: number; y: number; oldTile: number; newTile: number }
  | { type: 'entity:create'; entity: EntityDef }
  | { type: 'entity:delete'; entity: EntityDef; savedBehaviors: BehaviorRow[] }
  | { type: 'entity:move'; entityId: string; oldX: number; oldY: number; newX: number; newY: number }
  | { type: 'entity:rename'; entityId: string; oldName: string; newName: string }
  | { type: 'group'; records: UndoRecord[] }
  | {
    type: 'entity:updateVisual';
    entityId: string;
    oldSolid: boolean;
    oldSpriteId?: string;
    oldAnimationClipId?: string;
    newSolid: boolean;
    newSpriteId?: string;
    newAnimationClipId?: string;
  }
  | { type: 'behavior:row:add'; entityId: string; row: BehaviorRow }
  | { type: 'behavior:row:remove'; entityId: string; row: BehaviorRow }
  | { type: 'behavior:row:update'; entityId: string; before: BehaviorRow; after: BehaviorRow }
  | { type: 'effects:applyPreset'; before: MapEffectState; after: MapEffectState }
  | { type: 'effects:setFieldCoupling'; before: MapEffectState; after: MapEffectState }
  | { type: 'animation:anchor:add'; clipId: string; anchorName: string; keyframe: AnchorKeyframe }
  | { type: 'animation:anchor:move'; clipId: string; anchorName: string; oldKeyframe: AnchorKeyframe; newKeyframe: AnchorKeyframe }
  | { type: 'animation:anchor:remove'; clipId: string; anchorName: string; keyframe: AnchorKeyframe }
  | { type: 'entity:slot:attach'; entityId: string; attachment: SlotAttachment }
  | { type: 'entity:slot:detach'; entityId: string; attachment: SlotAttachment }
  | { type: 'entity:slot:setOcclusion'; entityId: string; slotName: string; oldHint: OcclusionHint; newHint: OcclusionHint }
  | { type: 'entity:setSpeed'; entityId: string; oldSpeed: number | undefined; newSpeed: number };
...
export class ProjectStore {
  ...
  selectedQuestNodeId: string | null = null;

  private undoStack: UndoRecord[] = [];
  private redoStack: UndoRecord[] = [];
  private activeBatch: UndoRecord[] | null = null;
  private validationDiagnostics: SemanticDiagnostic[] = [];
  ...
  getSelectedQuestNode(): QuestGraphNode | null {
    if (!this.selectedQuestNodeId) return null;
  ...
  private registerHandlers(): void {
    ...
    this.bus.on('effects:applyPreset', (cmd) => {
      ...
      const before: MapEffectState = {
        ...this.effectState,
        fieldLink: { ...this.effectState.fieldLink },
      };
      const after: MapEffectState = {
    ...
    this.bus.on('animation:anchor:add', (cmd) => {
      ...
      if (!clip.anchors) clip.anchors = {};
      if (!clip.anchors[cmd.payload.anchorName]) clip.anchors[cmd.payload.anchorName] = [];
    ...
    this.bus.on('animation:anchor:move', (cmd) => {
      ...
      if (cmd.payload.rot !== undefined) kf.rot = cmd.payload.rot;
      if (cmd.payload.flip !== undefined) kf.flip = cmd.payload.flip;
    ...
    this.bus.on('entity:slot:attach', (cmd) => {
      ...
      const attachment: SlotAttachment = {
    ...
    this.bus.on('entity:slot:detach', (cmd) => {
      const entity = this.entities.find((e) => e.id === cmd.payload.entityId);
      if (!entity || !entity.slots) return null;
  ...
  private applyInverse(record: UndoRecord): void {
    switch (record.type) {
      case 'group': {
      ...
      case 'entity:move': {
        ...
        if (ent) ent.position = { x: record.oldX, y: record.oldY };
        break;
      }
      ...
      case 'entity:rename': {
        const ent = this.entities.find((e) => e.id === record.entityId);
        if (ent) ent.name = record.oldName;
        break;
      }
      case 'entity:setSpeed': {
        const ent = this.entities.find((e) => e.id === record.entityId);
        if (ent) ent.speed = record.oldSpeed;
        break;
      }
      ...
      case 'behavior:row:update': {
        ...
        if (rows) {
          ...
          if (idx !== -1) rows[idx] = structuredClone(record.before);
        }
        break;
      }
      ...
      case 'animation:anchor:move': {
        ...
        if (clip?.anchors?.[record.anchorName]) {
          ...
          if (kf) Object.assign(kf, record.oldKeyframe);
        }
        break;
      }
      case 'animation:anchor:remove': {
        ...
        if (clip) {
          if (!clip.anchors) clip.anchors = {};
          if (!clip.anchors[record.anchorName]) clip.anchors[record.anchorName] = [];
      ...
      case 'entity:slot:attach': {
        ...
        if (ent?.slots) {
          ...
          if (idx !== -1) ent.slots.splice(idx, 1);
        }
        break;
      }
      ...
      case 'entity:slot:setOcclusion': {
        ...
        if (slot) slot.occlusionHint = record.oldHint;
        break;
      }
    }
  }
  ...
  private applyForward(record: UndoRecord): void {
    switch (record.type) {
      case 'group': {
      ...
      case 'entity:move': {
        const ent = this.entities.find((e) => e.id === record.entityId);
        if (ent) ent.position = { x: record.newX, y: record.newY };
        break;
      }
      case 'entity:updateVisual': {
        const ent = this.entities.find((e) => e.id === record.entityId);
      ...
      case 'entity:rename': {
        const ent = this.entities.find((e) => e.id === record.entityId);
        if (ent) ent.name = record.newName;
        break;
      }
      case 'entity:setSpeed': {
        const ent = this.entities.find((e) => e.id === record.entityId);
        if (ent) ent.speed = record.newSpeed;
        break;
      }
      case 'behavior:row:add': {
        if (!this.behaviors[record.entityId]) {
      ...
      case 'behavior:row:update': {
        ...
        if (rows) {
          ...
          if (idx !== -1) rows[idx] = structuredClone(record.after);
        }
        break;
      }
      case 'effects:applyPreset': {
      ...
      case 'animation:anchor:move': {
        ...
        if (clip?.anchors?.[record.anchorName]) {
          ...
          if (kf) Object.assign(kf, record.newKeyframe);
        }
        break;
      }
      case 'animation:anchor:remove': {
      ...
      case 'entity:slot:setOcclusion': {
        const ent = this.entities.find((e) => e.id === record.entityId);
        const slot = ent?.slots?.find((s) => s.slotName === record.slotName);
        if (slot) slot.occlusionHint = record.newHint;
        break;
      }
    }
  }

  private pushUndoRecord(record: UndoRecord): void {
    if (this.activeBatch) {
...
function isValidEffectState(v: unknown): v is MapEffectState {
  if (typeof v !== 'object' || v === null) return false;
  const obj = v as Record<string, unknown>;
  if (typeof obj['intensity'] !== 'number') return false;
  const pid = obj['activePresetId'];
  if (pid !== null && !isKnownPresetId(pid as string)) return false;
  const fieldLink = obj['fieldLink'];
  if (typeof fieldLink !== 'object' || fieldLink === null) return false;
  const link = fieldLink as Record<string, unknown>;
  const fieldId = link['fieldId'];
  if (fieldId !== null && !isKnownEffectFieldId(fieldId as string)) return false;
  if (typeof link['influence'] !== 'number') return false;
  return true;
}
