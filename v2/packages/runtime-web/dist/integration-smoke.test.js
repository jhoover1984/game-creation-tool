...
describe('INTEG-001 golden playable integration smoke', () => {
    ...
    it('new project -> tile -> entity -> save -> load roundtrip preserves authored state', () => {
        ...
        assert.equal(parsed.manifest.name, 'Smoke', 'saved JSON must contain project name');
        ...
        assert.equal(store2.tileLayers[0].data[3 * 10 + 2], 7, 'tile data must survive save/load roundtrip');
        ...
        assert.deepEqual(store2.entities[0].position, { x: 32, y: 48 }, 'entity authored position must survive save/load');
    });
    ...
    it('player entity moves right when moveX=1 (validates controlled movement)', () => {
        ...
        assert.ok(snapEntity.x > 16, `player x must increase after moveX=1 step (got ${snapEntity.x}, expected > 16)`);
        assert.ok(Math.abs(snapEntity.x - expectedX) < 0.01, `player x must be ~${expectedX} after 1 step (got ${snapEntity.x})`);
    });
...
//# sourceMappingURL=integration-smoke.test.js.map