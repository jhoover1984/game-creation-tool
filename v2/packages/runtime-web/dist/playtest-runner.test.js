...
function makeEntity(id, x, y, solid = true, tags = []) {
    return {
        id,
        name: id,
        position: { x, y },
...
function makeBehaviorRow(id, trigger, action) {
    return {
        id,
        label: id,
        enabled: true,
...
describe('PlaytestRunner', () => {
    ...
    it('player blocked by solid entity', () => {
        ...
        runner.init([makePlayer(0, 32), makeEntity('wall', 20, 32, true)], [emptyLayer()], 16);
        runner.enter();
        runner.setInput({ moveX: 1, moveY: 0 });
        for (let i = 0; i < 10; i++) {
            runner.step();
    ...
    it('player passes through non-solid entity', () => {
        const runner = new PlaytestRunner();
        runner.init([makePlayer(0, 32), makeEntity('ghost', 20, 32, false)], [emptyLayer()], 16);
        runner.enter();
        runner.setInput({ moveX: 1, moveY: 0 });
        for (let i = 0; i < 20; i++) {
            runner.step();
    ...
    it('entity with speed=200 uses 200 px/s (not hardcoded 120)', () => {
        const runner = new PlaytestRunner();
        const player = { ...makePlayer(), speed: 200 };
        runner.init([player], [emptyLayer()], 16);
        runner.enter();
    ...
    it('entity without speed field defaults to 120 px/s', () => {
        const runner = new PlaytestRunner();
        const player = { ...makePlayer() }; // no speed field
        runner.init([player], [emptyLayer()], 16);
        runner.enter();
        runner.setInput({ moveX: 1, moveY: 0, interact: false });
        const snapshot = runner.step();
...
//# sourceMappingURL=playtest-runner.test.js.map