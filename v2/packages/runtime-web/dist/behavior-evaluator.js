...
export class BehaviorEvaluator {
    maxTrace = 200;
    maxRowsPerEvaluate = 256;
    maxActionsPerRow = 16;
    traceBuffer = [];
    lastOverflow = { rowCapHit: false, actionCapHits: [] };
    /**
     ...
     */
    evaluate(behaviors, entities, trigger, triggerEntityIds) {
        // Reset overflow state for this call
        this.lastOverflow = { rowCapHit: false, actionCapHits: [] };
        const actionCapSeen = new Set(); // dedup key: `${entityId}:${rowId}`
        const produced = [];
        const now = Date.now();
        let evaluatedRows = 0;
        for (const entity of entities) {
            if (triggerEntityIds && !triggerEntityIds.has(entity.id))
                continue;
    ...
    // -- private helpers --
    evaluateConditions(row, entity, entities) {
        return row.conditions.map((cond) => {
            ...
            if (cond.type === 'always') {
                ...
                rawPass = false;
                reason = `condition "${cond.type}" not yet implemented (MVP)`;
            }
    ...
    evaluateActions(row, entityId, conditionsPassed, actionCapSeen) {
        return row.actions.map((action, index) => {
            ...
            return {
                ...
                type: action.type,
                dispatched: false,
                reason: `action "${action.type}" not yet implemented (MVP)`,
            };
        });
    }
...
//# sourceMappingURL=behavior-evaluator.js.map