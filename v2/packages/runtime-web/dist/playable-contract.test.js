...
describe('INTEG-CONTRACT-001 playable contract', () => {
    it('starter authored state + speed command + playtest tick + save/load invariants', () => {
        ...
        assert.ok((snap?.interactions ?? []).some((i) => i.actorId === player.id && i.targetId === npc.id), 'player should interact with nearby interactable in same tick');
...
//# sourceMappingURL=playable-contract.test.js.map