...
describe('AnimationPlayer', () => {
    ...
    it('pingpong mode reverses direction', () => {
        ...
        const frames = [player.currentFrame()];
        for (let i = 0; i < 8; i++) {
            player.tick(0.1);
    ...
    it('trigger transitions to new clip', () => {
        const player = new AnimationPlayer([idleClip(), attackClip()], [{ fromClip: 'idle', toClip: 'attack', condition: { type: 'onTrigger', trigger: 'attack' } }]);
        assert.equal(player.currentClip(), 'idle');
    ...
    it('onComplete transitions when clip finishes', () => {
        const player = new AnimationPlayer([attackClip(), idleClip()], [{ fromClip: 'attack', toClip: 'idle', condition: { type: 'onComplete' } }]);
        assert.equal(player.currentClip(), 'attack');
        for (let i = 0; i < 5; i++)
            player.tick(0.1);
        assert.equal(player.currentClip(), 'idle');
