...
test('ingestSemanticDiagnostics attaches fix actions for known codes', () => {
    const store = new DiagnosticStore();
    store.ingestSemanticDiagnostics([{ code: 'QUEST_START_NODE_MISSING', severity: 'error', path: '/nodes', message: 'Missing start' }], 'semantic');
...
test('ingestSemanticDiagnostics attaches fix actions for edit-time codes', () => {
    const store = new DiagnosticStore();
    store.ingestSemanticDiagnostics([{ code: 'EDIT_TILE_OUT_OF_BOUNDS', severity: 'error', path: '/tileLayers/layer-0/tiles', message: 'Out of bounds' }], 'editor');
    const diag = store.getAll()[0];
    assert.equal(diag.actions.length, 1);
...
//# sourceMappingURL=diagnostic-store.test.js.map